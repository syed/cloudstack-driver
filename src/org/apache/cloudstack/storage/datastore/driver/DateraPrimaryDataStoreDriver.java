...
public class DateraPrimaryDataStoreDriver implements PrimaryDataStoreDriver {
    ...
    private String getAppInstanceName(DataObject dataObject) {
        ...
        case VOLUME:
            ...
            // If the Folder is set then use that. It is possible that a volume's name set in Folder
            // maybe different from the one generated by using it's UUID. This happens in case of a volume
            // that has been migrated from a source to Datera. In this case Cloudstack creates a new volume
            // with a new UUID and copies the data and changes the UUID of the volume back to the UUID of the
            // original volume that the migration has been performed on. In this process, the volume name on Datera
            // is not changed and is saved in the folder field in the volume table.
            if (!StringUtils.isEmpty(volumeVo.getFolder())) {
        ...
        String appInstanceName = com.cloud.utils.StringUtils.join("-", name.toArray());
